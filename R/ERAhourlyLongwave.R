#' Estimates hourly incoming longwave radiation from ERA 3 hourly values
#' @name ERAhourlyLongwave
#' @description This function is called \emph{after} using the function \code{getNearestERAtimeseries}. This function deaccumulates the 12-hour cumulative values,using the function \code{ERAdeaccum} and interpolates the 3-hour values to hourly values, based on the extra-terrestrial hourly radiation.
...
ERAhourlyLongwave <- function(ERAstrd, strdColnum=1, ERAt2m, t2mColnum=1,
  ...
  # deaccumulate longwave and air temps
  ...
  first.datetime <- longwaveDeaccum$datetime[1]
  second.datetime <- longwaveDeaccum$datetime[2]
