...
#' @author Kevin Shook
#' @seealso \code{\link{WATCHdailyArealPrecip}}
#' @export
#'
#' @examples \dontrun{
...
WATCHgetWFDarealPrecip <- function(ncdfFile = '', minLon = 0, maxLon = 0, 
  ...
  # get specified range
  land <- land
  land_selected <- which((land$Longitude >= minLon) &
                        (land$Longitude <= maxLon) &
                        (land$Latitude >= minLat) &
                        (land$Latitude <= maxLat))
 
  selected_values <- all.values[land_selected,] * 
  ...
  precip <- data.frame(land$Longitude[land_selected], land$Latitude[land_selected], selected_values)
